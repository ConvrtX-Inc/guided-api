...
export class InboxService extends TypeOrmCrudService<Messages> {
  ...
  async getMessagesList(user_id: string){
    ...
    const uniqueSenderId = await this.inboxRepository.createQueryBuilder('inboxlist')
    .select([
      'inboxlist.sender_id',
    ])
    ...
    for(const senderId of uniqueSenderId){
      const messages = await this.inboxRepository.createQueryBuilder('inboxlist')
      .innerJoinAndMapMany('inboxlist.notification', Notification, 'notification', 'inboxlist.id = notification.message_id')
      .innerJoinAndMapMany('notification.user', User, 'user', 'user.id = inboxlist.sender_id::uuid')
      .select([
        'inboxlist.sender_id',
        'inboxlist.message',
