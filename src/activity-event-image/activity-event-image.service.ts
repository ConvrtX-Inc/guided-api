...
export class ActivityEventImageService extends TypeOrmCrudService<ActivityEventImage> {
  constructor(
    @InjectRepository(ActivityEventImage)
  ...
  async findManyEntities(options: FindOptions<ActivityEventImage>) {
    return this.activityEventImageRepository.find({
      where: options.where,
    });
  }
  async saveEntity(data: DeepPartial<ActivityEventImage>) {
    return this.activityEventImageRepository.save(
      this.activityEventImageRepository.create(data),
    );
  }
