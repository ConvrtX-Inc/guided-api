...
const userCrudServiceMockFactory: () => MockType<UsersCrudService> = jest.fn(
  () => ({
    findOneEntity: jest.fn((options: FindOptions<User>) => {
      return new User();
    }),
    // ...
  }),
...
describe('Users Service (Unit Test)', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(UsersCrudService)
      .useFactory({ factory: userCrudServiceMockFactory })
      .compile();
  ...
  describe('Update Phone No', () => {
    it('should successfully update', async () => {
      let findOneEntitySpy = jest
        .spyOn(crudService, 'findOneEntity')
        .mockImplementation(async (options: FindOptions<User>) => {
          return testUser;
        });
      let updateSpy = jest
    ...
    it('should fail if user id not specified', async () => {
      ...
      await expect(service.updatePhoneNo(null, 639173332626)).rejects.toThrow(
        HttpException,
      );
    ...
    it('should fail if phone_no not specified', async () => {
      ...
      await expect(service.updatePhoneNo('123', 0)).rejects.toThrow(
        HttpException,
      );
    ...
    it('should fail if User not Found', async () => {
      ...
      await expect(service.updatePhoneNo('123', 639173332626)).rejects.toThrow(
        HttpException,
      );
  ...
  describe('Update User', () => {
    ...
    it('should fail if password is not provided', async () => {
      ...
      await expect(service.update('123', testUser)).rejects.toThrow(
        HttpException,
      );
  ...
  describe('Update About Text', () => {
    ...
    it('should fail if user id not specified', async () => {
      ...
      await expect(
        service.updateAbout(null, 'a valid about text'),
    ...
    it('should fail if User not Found', async () => {
      ...
      await expect(
        service.updateAbout('123', 'a valid about text'),
      ).rejects.toThrow(HttpException);
  ...
  describe('Update Photo', () => {
    it('should successfully update photo', async () => {
      ...
      let findFileSpy = jest
        .spyOn(fileRepo, 'findOne')
    ...
    it('should fail if user id not specified', async () => {
      ...
      await expect(service.updatePhoto(null, '456')).rejects.toThrow(
        HttpException,
      );
    ...
    it('should fail if file id not specified', async () => {
      ...
      await expect(service.updatePhoto('123', null)).rejects.toThrow(
        HttpException,
      );
    ...
    it('should fail if User not Found', async () => {
      ...
      await expect(service.updatePhoto('123', '456')).rejects.toThrow(
        HttpException,
      );
