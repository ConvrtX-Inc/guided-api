...
export class NearbyActivitiesService {
  ...
  //HERE
  async nearbyActivitiesByUsersPreferedActivity(lat: number, lng: number, distance: number, id: string) {
  ...
  async nearbyDestinationsByUsersPreferedActivity(lat: number, lng: number, distance: number, id: string) {
    const user = await getRepository(User)
      .createQueryBuilder('users')
      .select('users.id, users.badge_id')
      .where("users.id = '" + id + "'")
  ...
  async nearbyPopularGuidesByUsersPreferedActivity(lat: number, lng: number, distance: number, id: string) {
    const user = await getRepository(User)
      .createQueryBuilder('users')
      .select('users.id, users.badge_id')
      .where("users.id = '" + id + "'")
