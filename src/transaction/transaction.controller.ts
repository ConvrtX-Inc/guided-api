...
export class TransactionController implements CrudController<Transaction> {
  ...
  public async getTransactionsByUserAndStatus(
  ...
  public async getTransactionsByGuideAndStatus(
    @Param('tour_guide_id') tour_guide_id: string,
    @Param('status') status: string,
  ) {
